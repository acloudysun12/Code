...
compute_cophenet <- function(M){
	...
	C = list();
	D = dim(M[[1]])[1] # this is "P", the number of columns in the NxP matrix 

	for(m in 1:length(M)){
		assignment = apply(M[[m]], 1, which.max) # find index of largest value per row of factor matrix
	...
	d1 = dist(Ds) # Euclidean distance between rows
	hc = hclust(d1, "ave")
	d2 = cophenetic(hc)
	coph = cor(d1, d2) # does NOT work if K = 1
