...
function(inputdir_data="C:/Users/aclou/Documents/Projects/Thesis/Code/data/", 
         X_mtx, W_mtx, num_facs, max_iters = 100, penalty_L = 0.1, penalty_F = 0.1){
  ...
  lambda1 = penalty_F
  outputdir = inputdir
  xfn=paste0(inputdir_data,'X_sim.txt')
  wfn=paste0(inputdir_data,'W_sim.txt')
...
For understanding conceptually. Simulate 200x10 matrix. 50% sparse on factors (F) and 50% sparse on loadings (L).
...
# replicate F+ (non-neg), 1 factor (px1), and also exp(1/2) so signal is much stronger than noise
...
for(K_iter in 1:length(K_vec)){
  ...
  for(iter in 1:length(F_penal_vec)){
    ...
    nnf_out = run_nnf(X_mtx = X_sim, W_mtx = W_sim, num_facs = K_val, 
                      max_iters = 50, penalty_L = penal_L, penalty_F = penal_F) # 20 iters max for testing
...
colnames(df_nnf_results) = c("Run", "K", "F_lambda1", "L_alpha1", 
...
df_nnf_results[df_nnf_results$Run == 9999, ]
...
For conceptual understanding how both matrix factorizations perform for slightly more complicated matrices, we simulate a 400x10 matrix with the true number of latent factors as 5. 25% sparse on F and 25% on L this time. Again, non-sparse factor values have exp(1/2) distribution (so the signals are strong enough to be noise).
...
Next, we select optimal K based on which K maximizes the cophonet correlation, as per Heyuan's section "Model Selection". Then, we select optimal sparsity values for L and F based on lowest correlation between factors. 

We use mean absolute pairwise correlation for selecting optimal sparsity parameters.

WHAT OTHER WAYS can we select sparsity parameters? What statistic to minimize correlation between factors?
...
HOW TO MEASURE HOW SIGNIFICANT IS RRMSE IMPROVEMENT? 

```{r}

print("'Optimal' selected run results, sn-spmf")
df_nnf_results_fac5[run_opt,]

print("Best sn-spmf run results based on true K = 5, sn-spmf")
df_nnf_results_fac5[run_opt_f5,]

print("All flash_backfit runs")
print(df_ebmf_results_f5)

print("Time taken for flash_backfit") 
print(df_ebmf_results_f5[run_opt_eb,])
